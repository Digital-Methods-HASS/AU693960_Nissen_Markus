...
ggplot(data = Western_Europe)+ 
  ...
  geom_col()+
  labs(title = "Homicides over time", y = "Homcides pr 100k people")
...
```{r Rearangement of homicides, eval = TRUE}
ggplot(data = Western_Europe) + 
  #... YOUR CODE
  aes(x = Year, y = homicides_per_100k, color=Entity)+
  geom_point()+
...
```{r Processing of danish monarchs, eval = TRUE}
# YOUR CODE HERE:
kongerakke <- read.csv2("data/kongedata.csv")

kongerakke

kongerakke$reign_length <- kongerakke$Reign_end_AD - kongerakke$Reign_start_AD

kongerakke$reign_midyear <- kongerakke$Reign_end_AD - ( kongerakke$reign_length / 2 )

ggplot(data = kongerakke, aes(x = reign_midyear, y = reign_length)) + 
  geom_smooth()+
  labs(title= "Reign Lengths and midyearsacross time", x = "Midyear of reign", y = "Length of Reign")
...
```{r Cleanup of homicide-data, eval = TRUE}
ggplot(data = Western_Europe) + 
  #... YOUR CODE
  aes(x = Year, y = homicides_per_100k, color=Entity)+
  geom_point()+
...
Technology as an integral part of life is a fundamental act of violence against the autonomous human subject. The fundamental destruction of privacy, and submersion of every aspect of our lives in technology has made the term violence in it of itself an outdated term, since the technology operating cannot operate outside of a binary construction of the world, and therefore cannot see anything but either violence or non-violence. The violence in that regard becomes simplified to the point of not even making sense, since the very definition of violence is fundamentally opposed to a binary understanding of the world. One of the many consequences of the industrial revolution. The "process of civiliziation" can also, like violence, not be understood af a binary, hence why the term is obsolete. I deny taking part in this neo-colonial narratavisation of the historical process.
